...
// LogAppenderBase::SharedPtr Logger::GetAppender(const std::string& appender_name) {
//     auto guard = std::unique_lock(shared_mutex_);
...
LogAppenderBase::LogAppenderBase(const bool is_async = false) : is_async_(is_async) {
...
void LogAppenderBase::Log(LogLevel::Level level, LogEvent::SharedPtr event) {
    if (is_async_ == false) {
        DoLog(level, event);
    } else {
        DoLogAsync(level, event);
    }
}

FileLogAppender::FileLogAppender(const std::string& file_name, const bool is_async) 
    ...
    if (is_async) {
        std::unique_ptr<BlockDeque<std::string>> new_log_buf(new BlockDeque<std::string>);
...
void FileLogAppender::DoLog(LogLevel::Level level, LogEvent::SharedPtr event) {
    ReopenFile();
    if (level >= level_) {
...
void FileLogAppender::DoLogAsync(LogLevel::Level level, LogEvent::SharedPtr event) {
    if (level >= level_) {
...
//void ConsoleLogAppender::DoLogAsync(LogLevel::Level level, LogEvent::SharedPtr event) {
//}


void ConsoleLogAppender::DoLog(LogLevel::Level level, LogEvent::SharedPtr event) {
    if (level >= level_) {
        switch (level) {
...
public:
    ...
    void Format(std::ostream& os, LogLevel::Level level, LogEvent::SharedPtr event) override {
        os << event->GetContent();
    }
};
...
void Formatter::PatternParse() {
    ...
    // fmt: the parameter of FormatItemBase's Constructor, which is the character in {} 
    ...
    static std::map<std::string,   
        std::function<FormatItemBase::SharedPtr(const std::string&)> > char_to_item_map = {
#define XX(STR, CLASS) \
        ...
        if (std::get<2>(i) == 0) {
            ...
            if (it == char_to_item_map.end()) {
                format_items_.push_back(
                    FormatItemBase::SharedPtr(new StringFormatItem(
